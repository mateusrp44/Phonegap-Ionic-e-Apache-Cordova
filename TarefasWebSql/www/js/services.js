...
.factory('TarefasService', function($q){
    ...
    function createDB(){
      ...
      try {
        db = indexedDB.openDatabase('tarefasDB', '1.0', 'db app ionic', 2 * 1024 * 1024);
        db.transaction = (function (tx) {
          console.dir(tx);
          tx.executeSql('CREATE TABLE IF NOT EXISTS tarefas (task_id INTEGER PRIMARY KEY AUTOINCREMENT, task_name varchar(200))');
        });
      }catch (err){
        alert('Erro: ' + err);
      }
